...
resource "azurerm_key_vault" "this" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "When rbac_authorization_enabled is false, provide at least one access policy."
    }

    precondition {
      condition     = !var.rbac_authorization_enabled || length(var.access_policies) == 0
      error_message = "Do not provide access_policies when rbac_authorization_enabled is true."
    }
  }
}
